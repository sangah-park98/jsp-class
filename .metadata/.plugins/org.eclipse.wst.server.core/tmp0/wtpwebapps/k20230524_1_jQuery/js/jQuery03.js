...
//	let imgs = document.getElementsByTagName('button');
//	for(let img of imgs) {
...
function classSelector() {
	$('.selClass').css('opacity','50%'); // 0.5도 가능
}
...
//   태그이름[속성이름~=문자열]: 문자열이 공백을 경계로 포함된 모든 요소를 선택한다. 단어포함(띄어쓰기로 구분)
//   태그이름[속성이름*=문자열]: 문자열이 포함되는 모든 요소를 선택한다. 문장포함
//   태그이름[속성이름$=문자열]: 문자열이 끝나는 모든 요소를 선택한다.
//   태그이름[속성이름^=문자열]: 첫 시작이 같은 모든 요소를 선택한다.
//   태그이름[속성이름!=문자열]: 정확한 그 단어와 일치하지 않은 모든 요소를 선택한다.
//   태그이름[속성이름|=문자열]: 지정된 문자열 또는 문자열에 -가 붙은 모든 요소를 선택한다.
...
//	$('img[title~=img02]').css('opacity', '0.2');
	$('img[title*=img02]').css('opacity', '0.2');
//	$('img[title$=04]').css('opacity', '0.2'); // +는 적용 불가
...
//	위의 <input> 태그 요소는 checked 속성을 가지고 있으며 속성값은 "checked"이다.
//	만약에 위의 checkbox가 선택되었을 때 checked 속성의 property는 "true" 이다. 

//	속성과 프로퍼티 설정
//   .attr(): 인수로 지정된 속성의 속성값을 얻어온다.
