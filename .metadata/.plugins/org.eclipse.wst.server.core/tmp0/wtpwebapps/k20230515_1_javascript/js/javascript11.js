...
//	멤버 변수 초기화
	...
	this.sound = sound;
//	멤버 함수 
//	함수를 클래스 내부에서 선언하면 클래스의 객체가 생성될 때마다 함수가 매번 만들어진다.
	this.say = function() {
...
//	함수를 클래스 내부에서 선언하면 클래스의 객체가 생성될 때마다 같은 함수가 만들어진다.
...
console.log(dog2);
dog2.say();
const cat2 = new Animal2('고양이', '야옹이', '왈왈');
console.log(cat2);
cat2.say();
console.log('-----------------------------------');
...
//	prototyoe으로 함수를 선언하지 않고 클래스 외부에서 만든 함수를 각각의 객체에 넣어줄 수 있다.
function say() {
	console.log(this.sound);
}
