...
function testDate1() {
	...
	today.innerHTML = date; // Mon May 22 2023 13:12:05 GMT+0900 (한국 표준시)
	today.innerHTML = date.toDateString(); // Mon May 22 2023
	today.innerHTML = date.toLocaleString(); // 2023. 5. 22. 오후 1:13:52
	today.innerHTML = date.toLocaleDateString(); // 2023. 5. 22.
	today.innerHTML = date.toLocaleTimeString(); // 오후 1:14:46
}
...
function testDate5() {
//	문자열 형태로 수료 날짜를 얻어와서 날짜 데이터로 변환한다.
	let date2 = document.getElementById('date2').value; // string 문자 타입
	let endDate = new Date(date2); // 문자 -> 날짜 형식으로 변환

//	문자열 형태로 지정 날짜를 얻어와서 날짜 데이터로 변환한다.
...
//	날짜/시간 데이터를 연산하면 두 시간 데이터 연산 결과를 밀리초 단위로 계산한다.
	console.log(result2); // 엄청 엄청 큰 수가 나오는데 연산 결과의 시, 분, 초, 밀리초가 합한 수가 나오기 때문!
