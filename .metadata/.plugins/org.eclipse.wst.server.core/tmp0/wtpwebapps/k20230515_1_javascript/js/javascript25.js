function numTest(num) {
	console.log(num);
	console.log(typeof num); // text 상자 안에 있는 건 무조건 문자열을 반환한다. string
...
//	덧셈을 하기 위해서는 숫자로 변환한 후 연산을 해야 한다.
//	Number() 함수는 인수로 지정된 문자열을 숫자로 바꿔준다. => 정수, 실수를 구분하지 않는다.
	console.log(Number('123') + Number(num));
	console.log(Number('3.14') + Number(num)); // 103.14
	
//	'100a'와 같이 인수로 지정된 문자열에 숫자로 변환할 수 없는 문자가 있을경우 parseInt() 함수나
