...
function strTest3() {
	let str = prompt('이름이 뭐예요?', '숫자는 입력하지 마세요.');
//	console.log(typeof str);
	
//	isNaN(): 인수로 지정된 값이 숫자가 아니면 true, 숫자면 false를 리턴한다.
//	100a도 true
//	console.log(isNaN(str));
//	console.log(isNaN('aaa' + isNaN(str))); // 문자열이 더해지면 무조건 true
...
//	'===', '!=='는 데이터 타입과 데이터를 모두 비교한다. '1'과 1을 다른 값으로 취급한다.
	...
	if(num == str) {
		...
		console.log('데이터 타입까지 비교하므로 다르다.');
	}
	console.log('-----------------------');
	...
	let oldName = '홍길동';
	console.log(typeof oldName); // string
	let newName = new String('홍길동'); 
	console.log(typeof newName); // object
	
	console.log(oldName == newName ? '같다' : '다르다'); // 같다
	console.log(oldName === newName ? '같다' : '다르다'); // 다르다
...
//	substring(a, b): 문자열에서 index가 a인 문자부터 b-1 문자까지 추출한다.
 	let today = '2023-05-22 (월) 2023-04-03';
	let sola = today.substring(0, today.indexOf(' '));
	let lunar = today.lastIndexOf(today.lastIndexOf(' ') + 1);
