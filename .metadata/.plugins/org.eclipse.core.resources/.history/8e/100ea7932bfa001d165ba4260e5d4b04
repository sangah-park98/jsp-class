...
//	javascript는 value 속성을 사용해서 [~~~~~.value] 형태로 값을 얻어오고
//	[~~~~~.value='값'] 형태로 값을 넣어준다.
//	jQuery는 value 속성 대신 val() 메소드를 사용해서 [~~~~~.val()] 형태로 값을 얻어오고
//	[~~~~~.val('값')] 형태로 값을 넣어준다.
...
//	아래와 같이 사용하면 input 태그의 type 속성이 text인 요소가 여러 개 있다 하더라도 첫 번째
//	요소의 값만 얻어온다.
...
//	jQuery는 innerHTML 역할을 하는 html() 메소드나 innerText, textContent 역할을 하는 text()
//	메소드로 태그 내부에 텍스트를 넣어준다.
//	html() 메소드 태그를 지원하고 text() 메소드는 태그를 지원하지 않는다.
	$('#a').html('<h1>' + text + '</h1>');
//	$('#a').text('<h1>' + text + '</h1>');
}
