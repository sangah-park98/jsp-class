...
<%--
	...
	<%= ~ %>: 표현식 => 변수에 저장된 결과나 연산결과를 출력한다. => EL(${ ~ })로 대체해서 사용한다.
...
<!-- 
	html은 spacebar를 여러 번 입력해도 한 칸밖에 떨어지지 않고, enter 키를 여러 번 눌러도 줄이 변경되지 않는다.
...
<!-- 
	...
	이름을 쓴다. 아무것도 안 쓰거나 "?"만 입력하면 현재 페이지를 요청한다.
 ...
 <form action="myInfoOK.jsp" method="post">
 	<label>이름: <input type = "text" name="name"> <!--name은 변수명 --></label><br>
	...
	radio와 checkbox는 같은 그룹끼리 반드시 같은 이름의 name 속성을 지정해서 그룹으로 묶어야 한다.
 	...
 	checked="checked" 속성은 radio에서는 맨 마지막에 지정한 속성으로 취급되고 
 	checkbox에서는 지정하는대로 선택된다.
	...
	<input type = "checkbox" name="hobbies" value="바둑" checked="checked">바둑
	<input type = "checkbox" name="hobbies" value="낚시">낚시
 	...
 	<!-- 
 		...
 		콤보상자는 첫번째 <option> 태그에 입력한 내용이 기본적으로 표시되는데 selected="selected"
	...
	<select name="trip" style="width: 200px; height: 30px">
 	...
 	<!-- 
 		...
 		목록상자에 표시되는 아이템의 개수는 4개가 기본값으로 표시되고 size 속성을 지정하면 지정한만큼 표시된다.
 	 -->
 	...
 	<input type="button" value="그냥버튼" onclick="alert('화이팅')">
 	
 	<button type="submit">정보보기</button>
 	<button type="reset">정보보기</button>
 	<button type="button" onclick="alert('파이팅')">정보보기</button>
