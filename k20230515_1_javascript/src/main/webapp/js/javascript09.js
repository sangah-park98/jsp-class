...
//	splice(): 배열에서 index번째 위치부터 지정된 개수만큼 뽑아내서 배열로 리턴한다. 
//	배열명.splice(index, 개수)
...
//	배열에 데이터 추가, 제거 함수
//	push(): 배열의 맨 뒤에서 인수로 지정한 데이터를 추가한다.
//	unshift(): 배열의 맨 앞에 인수로 지정한 데이터를 추가한다.
//	pop(): 배열의 맨 뒤 데이터를 가져온 후 제거한다.
...
const conated = array.concat(array2, array3);
console.log(conated);
console.log('--------------------------------concat()');

//	join(): 배열 요소들 사이에 인수로 지정된 문자열을 삽입해서 문자열로 만들어 리턴한다.
//	자바의 String.format()처럼 내 마음대로 문자 형식을 지정하는 것
//	배열명.join(['문자열'])

const array4 = ['홍길동', '임꺽정', '장길산', '일지매'];
console.log(typeof array4.join());
console.log(array4.join()); // 인수를 지정하지 않으면 ','가 기본으로 지정된다.
